...
WXPay.mix('getBrandWCPayRequestParams', function(order, fn){
	...
	this.createUnifiedOrder(order, function(err, data){
		console.log(`third wx pay:${JSON.stringify(data)}`);
		var reqparam = {
		...
		if(pay_info){ 
			...
			reqparam.nonceStr = pay_info.nonceStr;
			reqparam.package = pay_info.package;
		}
		else{
			reqparam.package = 'sucess'
		}
		
		reqparam.paySign = _this.sign(reqparam);
		reqparam.return_code = data.return_code;
		reqparam.return_msg = data.return_msg;
		reqparam.result_code = data.result_code;
		reqparam.err_code = data.err_code;
		reqparam.err_code_des = data.err_code_des;
		reqparam.token_id = data.token_id;
		fn(err, reqparam);
	});
});
...
WXPay.mix('useWXCallback', function(fn){
	...
	return function(req, res, next){
		var _this = this;
		res.success = function(){ res.end('success'); };
		res.fail = function(){ res.end('fail'); };
